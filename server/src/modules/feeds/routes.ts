...
export async function feedRoutes(app: FastifyInstance) {
  ...
  app.get('/search', async (req, reply) => {
    try {
      ...
      const q = (query.q as string) as string;
  ...
  app.get('/hot', async (req, reply) => {
    ...
    const { limit, offset } = parsed.data as any;
    ...
    try {
      ...
      try {
        ...
        if (tbl[0]?.t) {
          const tableRef = tbl[0].t as string;
          let { rows } = await client.query(
            `SELECT hs.ts_code, d.name, d.industry, d.exchange,
          ...
          rows = rows.map((r:any) => {
            ...
            if ((pe === null || pe === undefined) && price && eps) pe = Number((price / eps).toFixed(2));
            if ((pb === null || pb === undefined) && price && bps) pb = Number((price / bps).toFixed(2));
            delete r.valuation;
  ...
  app.get('/ai/forecast/:ts_code', async (req, reply) => {
    try {
      ...
      if (!parsedP.success || !parsedQ.success) return reply.code(400).send({ error: 'Invalid params' });
      const { ts_code } = parsedP.data as any;
      const { pred_len, lookback } = parsedQ.data as any;
  ...
  app.post('/auto-import/:ts_code', async (req, reply) => {
    ...
    try {
      ...
      const runPy = (script: string, args: string[] = [], retries = 2) =>
    ...
    try {
      ...
      try {
        ...
        const overviewResult = await client.query(
          'SELECT COUNT(*) as count FROM dim_stock WHERE ts_code = $1',
          [ts_code]
        );
        status.has_overview = parseInt(overviewResult.rows[0].count) > 0;

        // 检查K线数据
        const klineResult = await client.query(
          'SELECT COUNT(*) as count FROM prices_ohlcv WHERE ts_code = $1',
          [ts_code]
        );
        ...
        const valuationResult = await client.query(
          'SELECT COUNT(*) as count FROM valuations WHERE ts_code = $1',
          [ts_code]
        );
        ...
        const aiResult = await client.query(
          'SELECT COUNT(*) as count FROM ai_scores WHERE ts_code = $1',
          [ts_code]
        );
        status.has_ai_score = parseInt(aiResult.rows[0].count) > 0;

        // 检查DCF数据（DCF数据存储在valuations表中，method='DCF'）
        const dcfResult = await client.query(
          'SELECT COUNT(*) as count FROM valuations WHERE ts_code = $1 AND method = $2',
          [ts_code, 'DCF']
        );
  ...
  app.post('/ai/insight', async (request, reply) => {
    try {
      const { ts_code, date } = request.body as { ts_code: string; date?: string };
      ...
      const cached = await pgPool.query(
        'SELECT cache_data FROM ai_cache WHERE cache_key = $1 AND expires_at > NOW()',
        [cacheKey]
      );
      ...
      `, [
        '/ai/insight',
        (request.body as any)?.ts_code || null,
  ...
  app.post('/ai/news', async (request, reply) => {
    try {
      const { news_text, news_url, ts_codes } = request.body as { 
        news_text: string; 
        news_url?: string; 
        ts_codes?: string[] 
      };
  ...
  app.post('/ai/strategy', async (request, reply) => {
    try {
      const { portfolio, constraints, strategy_type } = request.body as {
        portfolio: any[];
        constraints: any;
  ...
  app.get('/ai/insights/:ts_code', async (request, reply) => {
    try {
      const { ts_code } = request.params as { ts_code: string };
