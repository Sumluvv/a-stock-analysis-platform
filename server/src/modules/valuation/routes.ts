...
export async function valuationRoutes(fastify: FastifyInstance) {
  // 列出可用估值方法与默认参数
  fastify.get('/methods/:ts_code', async (request: FastifyRequest<{ Params: { ts_code: string } }>, reply: FastifyReply) => {
    ...
    try {
      ...
      fastify.log.error(e as any); return reply.code(500).send({ error: 'Failed to load methods' });
    }
  });

  // 通用计算接口
  fastify.post('/calc/:ts_code', async (request: FastifyRequest<{ Params: { ts_code: string }, Body: any }>, reply: FastifyReply) => {
    const { ts_code } = request.params;
    ...
    try {
      ...
      try {
        ...
        const resp: any = { ts_code, method, current_price: currentPrice };
        if (method === 'PE') {
      ...
      fastify.log.error(e as any); return reply.code(500).send({ error: 'Calculation failed' });
    }
  });
  // 获取股票估值信息
